...
local ActiveLineActors = {}
local LineActorPool = {}
local CachedMesh = nil
local CachedMaterial = nil

local function GetCachedMesh()
	if not CachedMesh then
		CachedMesh = UE.UStaticMesh.Load("/Engine/BasicShapes/Cube.Cube")
	end
	return CachedMesh
end

local function GetCachedMaterial(lineComp)
	if not CachedMaterial then
		CachedMaterial = lineComp:GetMaterial(0)
	end
...
function DrawLine(x1, y1, z1, x2, y2, z2, r, g, b, a, duration, thickness)
	...
	local lineActor = GetLineActor(world)
	if not lineActor then
		Framework.Debugging:LogError("Failed to get line actor")
		return
	end

	local lineComp = lineActor:GetComponentByClass(UE.UStaticMeshComponent)
	if not lineComp then
		Framework.Debugging:LogError("Line actor missing mesh component")
		return
	end

	local startVec = UE.FVector(x1, y1, z1)
	local endVec = UE.FVector(x2, y2, z2)
	local center = (startVec + endVec) * 0.5
	local dir = UE.FVector(endVec.X - startVec.X, endVec.Y - startVec.Y, endVec.Z - startVec.Z)

	local length = math.sqrt(dir.X ^ 2 + dir.Y ^ 2 + dir.Z ^ 2)
	if length == 0 then
		length = 0.01
	end
	dir.X, dir.Y, dir.Z = dir.X / length, dir.Y / length, dir.Z / length

	local yaw = math.atan(dir.Y, dir.X) * 180 / math.pi
	local pitch = math.asin(dir.Z) * 180 / math.pi
	local rotator = UE.FRotator(pitch, yaw, 0)

	lineActor:K2_SetActorLocation(center, false, nil, true)
	lineActor:K2_SetActorRotation(rotator, false)
	lineComp:SetWorldScale3D(UE.FVector(length / 100.0, thickness / 100.0, thickness / 100.0))

	table.insert(ActiveLineActors, lineActor)
...
function DrawPoly(v1, v2, v3, r, g, b, a, duration, thickness)
	...
	DrawLine(v2.X, v2.Y, v2.Z, v3.X, v3.Y, v3.Z, r, g, b, a, duration, thickness)
	DrawLine(v3.X, v3.Y, v3.Z, v1.X, v1.Y, v1.Z, r, g, b, a, duration, thickness)
end
...
function PolyZone:draw(forceDraw)
	if not forceDraw and not self.debugPoly and not self.debugGrid then
		return
	end
...
local function _calculateGridCellPoints(cellX, cellY, poly)
	...
	return {
		UE.FVector2D(x, y),
		UE.FVector2D(x + gridCellWidth, y),
		UE.FVector2D(x + gridCellWidth, y + gridCellHeight),
		UE.FVector2D(x, y + gridCellHeight),
		UE.FVector2D(x, y),
	}
end
		...
		if #yValues >= 2 then
			...
			for i = 1, #yValues do
				...
				if y - lastY > poly.gridCellHeight + 0.01 then
					lines[#lines + 1] = { min = UE.FVector2D(x, minY), max = UE.FVector2D(x, lastY) }
					minY = y
				elseif i == #yValues then
					-- If at the last point, create a line between minY and the last point
					lines[#lines + 1] = { min = UE.FVector2D(x, minY), max = UE.FVector2D(x, y) }
				end
				lastY = y
			end
		end
		...
		if #xValues >= 2 then
			...
			for i = 1, #xValues do
				...
				if x - lastX > poly.gridCellWidth + 0.01 then
					lines[#lines + 1] = { min = UE.FVector2D(minX, y), max = UE.FVector2D(lastX, y) }
					minX = x
				elseif i == #xValues then
					lines[#lines + 1] = { min = UE.FVector2D(minX, y), max = UE.FVector2D(x, y) }
				end
				lastX = x
			end
		end
...
local function _createGrid(poly, options)
	...
	Timer.CreateThread(function()
		...
		if options.debugGrid then
			...
			print("[PolyZone] Debug: Grid Coverage at " .. coverage .. "% with " .. poly.gridDivisions .. " divisions. Optimal coverage for memory usage and startup time is 80-90%")
...
local function _calculatePoly(poly, options)
	if not poly.min or not poly.max or not poly.size or not poly.center or not poly.area then
		...
		poly.min = UE.FVector2D(minX, minY)
		poly.max = UE.FVector2D(maxX, maxY)
