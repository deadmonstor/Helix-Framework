...
---@param name string|boolean|function
function Hook:Remove(event_name, name)
	...
	local notValid = isnumber(name) or isbool(name) or isfunction(name)
...
function Hook:Add(event_name, name, func, priority)
	...
	local notValid = name == nil or isnumber(name) or isbool(name) or isfunction(name)
	if not isstring(name) and notValid then
	...
	if isnumber(priority) and priority ~= nil then
...
function Hook:ProtectedCall(event_name, ...)
	local event = events[event_name]
	if not event then -- fast path
		return
	end
